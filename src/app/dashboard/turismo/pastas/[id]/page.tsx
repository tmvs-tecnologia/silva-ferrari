...
import { ArrowLeft, Folder, Pencil, Trash2, Eye, ChevronsUpDown, Check, Globe, Plane, Clock, CheckCircle2, Calendar, Circle, User } from "lucide-react";
...
export default function PastaTurismoDetail({ params }: { params: { id: string } }) {
  ...
  const getVistoStepTitle = (index: number) => {
    const steps = getVistoSteps();
    ...
    const loadAssignments = async () => {
      const entries = await Promise.all(
        records.map(async (r: any) => {
          ...
          try {
            // Note: Keeping moduleType=vistos because assignments are likely stored with 'vistos' module type for the table 'vistos'
                ...
                <Button variant="outline" className="h-9 w-[320px] justify-between">
                  <span className="truncate">
                    {(() => {
                      ...
                      return sel ? `${sel.client_name}` : 'Selecione um cadastro';
                    })()}
                  </span>
              ...
              <PopoverContent className="p-0 w-[420px]">
                <Command>
                  ...
                  <CommandList>
                    ...
                    <CommandGroup>
                      {candidates.map((v) => (
                        <CommandItem key={v.id} value={`${v.client_name}`} onSelect={() => setSelectedAddId(String(v.id))}>
                    ...
                    <div className="space-y-3 flex-1">
                      ...
                      <div className="grid gap-2 text-sm text-slate-700 dark:text-slate-300">
                        ...
                        <div className="flex items-center gap-2 w-full">
                          ...
                          <span className={getStatusFinalClass(String(v.statusFinal || ""))}>{(() => {
                            const s = String(v.statusFinal || "");
                            if (!s) return "-";
                            if (s === "Outro") return String(v.statusFinalOutro || "Outro");
                            return s;
                          })()}</span>
                        </div>
                        <div className="flex items-center gap-2">
